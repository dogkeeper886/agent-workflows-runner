...
export interface Judgment {
  ...
  /** Deterministic cross-check: claims in the answer that the live tool result does
   *  not support (the verifier's PASS cannot stand if this is non-empty). */
...
export interface RunConfig {
  ...
  dryRun: boolean;
  /** Also run the agent judge (opt-in via JUDGE_MODE=dual); default is simple-only */
  dualMode: boolean;
